...
void TestInterface::updateVelLimit(const Eigen::VectorXf &vm){
    ((CCCBTrajOptPlanner*)planner_)->setVelLimit(vm);
}

void TestInterface::updateAccLimit(const Eigen::VectorXf &am){
    ((CCCBTrajOptPlanner*)planner_)->setAccLimit(am);
}

void TestInterface::updateJerkLimit(const Eigen::VectorXf &jm){
